...
- Portfolio stack: **React 19, Vite 7, Tailwind CSS v4, Framer Motion, three.js + UnrealBloom, GSAP, Lenis, Radix UI, wouter, react-hook-form + zod.**
- Key dirs (all under `artifacts/portfolio/src/`):
  - `pages/Home.tsx` — thin theme fork: `theme === "wizarding" ? <WizardingHome/> : <ModernHome/>`
  - `components/modern/**` (+ `modern/modern.css`) — the dark/light "Liquid Glass / Aurora" redesign
  - `components/modern/JourneyScene.tsx` + `components/modern/journey/**` — fixed scroll-driven 3D neural journey (three.js)
...
- Theme isolation is by page-level fork (`Home.tsx`): `ModernHome` (with Lenis + JourneyScene) only mounts on dark/light; wizarding gets none of it. Keep it that way.
